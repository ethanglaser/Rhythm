...
void RN4020_send(RN4020* rn4020, const char* line) {
  ...
  HAL_USART_Transmit(rn4020->uart, (uint8_t*)line, strlen(line), RN4020_TIMEOUT);
  HAL_USART_Transmit(rn4020->uart, (uint8_t*)&newLineCh, 1, RN4020_TIMEOUT);
}
...
HAL_StatusTypeDef _RN4020_writeServerCharacteristic(RN4020* rn4020, const uint8_t* uuid, uint8_t uuidLen, const uint8_t* data, uint32_t dataLength) {
  ...
  sprintf(line, "SUW,");
  HAL_USART_Transmit(rn4020->uart, (uint8_t*)line, strlen(line), RN4020_TIMEOUT);
  HAL_USART_Transmit(rn4020->uart, (uint8_t*)uuidStr, strlen(uuidStr), RN4020_TIMEOUT);
  strcpy(line, ",");
  HAL_USART_Transmit(rn4020->uart, (uint8_t*)line, strlen(line), RN4020_TIMEOUT);
  for (uint32_t i = 0; i < dataLength; i++) {
    sprintf(line, "%02X", data[i]);
    HAL_USART_Transmit(rn4020->uart, (uint8_t*)line, 2, RN4020_TIMEOUT);
  }
  HAL_USART_Transmit(rn4020->uart, (uint8_t*)&newLineCh, 1, RN4020_TIMEOUT);
  return _RN4020_waitForReadyState(rn4020);
}
